...
def GetDataCallback(cb_DeviceIndex, cb_CANIndex, cb_Len):
    ...
    if(DataNum > 0):
        ...
        for i in range(0,DataNum):
            ...
            #m.z print("--CAN_ReceiveData.Data:",end='')
            print("--CAN_ReceiveData.Data:")
            for j in range(0,CAN_ReceiveData[i].DataLen):
                print("%02X "%CAN_ReceiveData[i].Data[j])
            print("")
...
if(CAN_GET_BOARD_INFO == 1):
    ...
    if(nRet == ControlCAN.STATUS_ERR):
        ...
        for i in range(0, len(CAN_BoardInfo.SerialNumber)):
            print("%02X"%CAN_BoardInfo.SerialNumber[i])
        print("")
else:
    # Open device
...
if(CAN_SEND_DATA == 1):
    ...
    for j in range(0, SEND_FRAME_COUNT):
        nRet = ControlCAN.VCI_Transmit(DevType,DeviceIndex,CANIndex,byref(CAN_SendData), SEND_FRAME_COUNT)
        if(nRet == ControlCAN.STATUS_ERR):
