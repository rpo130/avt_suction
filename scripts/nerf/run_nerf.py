...
from nerf.load_avt import load_avt_data
import pathlib
...
def render_path(render_poses, hwf, K, chunk, render_kwargs, gt_imgs=None, savedir=None, render_factor=0):
    ...
    dex_depths = np.stack(dex_depths, 0)
...
def run_test(config_path):
    ...
    hwf, K, imgs, poses = load_avt_data(args.datadir, False)
    torch.set_default_tensor_type('torch.cuda.FloatTensor')
    rgb,_,_,depth = render_single(args, poses[0], K, hwf)
...
if __name__=='__main__':
    rgb, depth = run_test('./nerf-pytorch/configs/avt_data_glass_13.txt')
    plt.imshow(depth)
    plt.show()
